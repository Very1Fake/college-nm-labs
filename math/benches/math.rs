...
use criterion::{criterion_group, Criterion, criterion_main, black_box, BenchmarkGroup, measurement::Measurement};

use nm_math::{parser::parse, expression::{Evaluable, Expr}, variable::{Scope, Var}};

fn settings<'a>(mut g: BenchmarkGroup<'a, impl Measurement>) -> BenchmarkGroup<'a, impl Measurement> {
...
fn main_bench(c: &mut Criterion) {
    ...
    scope.insert(Var::new("x", 2.0));

    let mut g = settings(c.benchmark_group("main"));
    g.bench_function("math", |b| b.iter_with_large_drop(|| black_box(expr_math.eval(&scope))));
    g.bench_function("rust", |b| b.iter(|| expr_rust(black_box(2.0))));
    g.finish();
}

fn neg_bench(c: &mut Criterion) {
    let expr_math_old = (-2.0 * Expr::var("x")).sin() * -1.0;  // Will be fixed in near future
    let expr_rust_old = |x: f64| (-2.0 * x).sin() * -1.0;  // Old style
    let expr_rust_new = |x: f64| -(-2.0 * x).sin();  // New style (will be implemented soon)
    let mut scope = Scope::default();
    scope.insert(Var::new("x", 1.0));

    let mut g = settings(c.benchmark_group("neg"));
    g.bench_function("math_old", |b| b.iter_with_large_drop(|| black_box(expr_math_old.eval(&scope))));
...
fn parse_bench(c: &mut Criterion) {
    let script = "-4.5x^2 * (sin(x) + 2x) * (sqrt(12) / -x)";

    c.bench_function("parse", |b| b.iter(|| parse(black_box(script)).unwrap()));
}
